...
public class ValidacionesLogin implements Validator {
    ...
    public void validate(FacesContext context, UIComponent component, Object value) throws ValidatorException {
        ...
        if (nombreUsuario.length() == 0) {
            throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR, "ERROR", "ingrese usuario"));
        } else {
            String expresionLetras = "[a-zA-ZñÑáéíóúÁÉÍÓÚ]+$";
            boolean aceptable = Pattern.matches(expresionLetras, nombreUsuario);
            if (!aceptable) {
                throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR, "ERROR", "Solo Letras"));
